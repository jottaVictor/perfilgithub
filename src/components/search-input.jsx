...
export default function SearchInput(
    ...
    const handleSearchHistory = (value) => {
        if(historyRef.current)
            historyRef.current.style.pointerEvents = 'none'
        historyData.onSearch(value)
        setTimeout(() => {
            if (historyRef.current) {
                historyRef.current.style.pointerEvents = 'auto';
            }
        }, 100)
    }
    ...
    return(
        <div className="search-input">
            ...
            <div ref={historyRef} className="content-history">
                <SearchHistory onSearch={handleSearchHistory} onDelete={historyData.onDeleteHistory} history={historyData.history} propToShow={historyData.propToShow} propToSearch={historyData.propToSearch} deletedField={historyData.deletedField}></SearchHistory>
            </div>
        </div>
    )
}